...
public:
  ...
  void process(const float *const *src, float *const *dst, unsigned left,
               unsigned right) const override {
...
public:
  GammaOperationC(gamma_func func, float prescale, float postscale)
      : m_func{func}, m_prescale{prescale}, m_postscale{postscale} {}
...
public:
  AribB67OperationC(double kr, double kg, double kb, float scale)
      : m_kr{static_cast<float>(kr)}, m_kg{static_cast<float>(kg)},
        m_kb{static_cast<float>(kb)}, m_scale{scale} {}
...
public:
  CLToRGBOperationC(gamma_func to_gamma, gamma_func to_linear, double kr,
                    double kg, double kb, float scale)
      : m_func{to_linear}, m_kr{static_cast<float>(kr)},
        m_kg{static_cast<float>(kg)}, m_kb{static_cast<float>(kb)}, m_nb{},
        m_pb{}, m_nr{}, m_pr{}, m_scale{scale} {
    EnsureSinglePrecision x87;
...
public:
  CLToYUVOperationC(gamma_func func, double kr, double kg, double kb,
                    float scale)
      : m_func{func}, m_kr{static_cast<float>(kr)},
        m_kg{static_cast<float>(kg)}, m_kb{static_cast<float>(kb)}, m_nb{},
        m_pb{}, m_nr{}, m_pr{}, m_scale{scale} {
    EnsureSinglePrecision x87;
...
std::unique_ptr<Operation> create_matrix_operation(const Matrix3x3 &m,
                                                   CPUClass cpu) {
...
create_gamma_operation(const TransferFunction &transfer,
...
#endif
  if (!ret)
    ret = std::make_unique<GammaOperationC>(transfer.to_gamma,
...
std::unique_ptr<Operation>
create_inverse_gamma_operation(const TransferFunction &transfer,
                               const OperationParams &params, CPUClass cpu) {
  std::unique_ptr<Operation> ret;
...
#endif
  if (!ret)
    ret = std::make_unique<GammaOperationC>(transfer.to_linear, 1.0f,
                                            transfer.to_linear_scale);

  return ret;
}

std::unique_ptr<Operation>
create_arib_b67_operation(const Matrix3x3 &m, const OperationParams &params) {
  zassert_d(!params.scene_referred, "must be display-referred");

  TransferFunction func = select_transfer_function(
      TransferCharacteristics::ARIB_B67, params.peak_luminance, false);
  return std::make_unique<AribB67OperationC>(m[0][0], m[0][1], m[0][2],
                                             func.to_gamma_scale);
}
...
create_inverse_arib_b67_operation(const Matrix3x3 &m,
  ...
  TransferFunction func = select_transfer_function(
      TransferCharacteristics::ARIB_B67, params.peak_luminance, false);
  return std::make_unique<AribB67InverseOperationC>(m[0][0], m[0][1], m[0][2],
                                                    func.to_linear_scale);
}

std::unique_ptr<Operation>
create_cl_yuv_to_rgb_operation(const ColorspaceDefinition &in,
                               const ColorspaceDefinition &out,
                               const OperationParams &params, CPUClass cpu) {
  zassert_d(in.primaries == out.primaries, "primaries mismatch");
  zassert_d((in.matrix == MatrixCoefficients::REC_2020_CL ||
             in.matrix == MatrixCoefficients::CHROMATICITY_DERIVED_CL) &&
                in.transfer == TransferCharacteristics::REC_709,
            "must be 2020 CL");
  zassert_d(out.matrix == MatrixCoefficients::RGB &&
                out.transfer == TransferCharacteristics::LINEAR,
            "must be linear RGB");

  std::cout << "Using CL YUV to RGB" << '\n';

  // CL is always scene-referred.
  TransferFunction func =
      select_transfer_function(in.transfer, params.peak_luminance, true);
...
std::unique_ptr<Operation>
create_cl_rgb_to_yuv_operation(const ColorspaceDefinition &in,
                               const ColorspaceDefinition &out,
                               const OperationParams &params, CPUClass cpu) {
  zassert_d(in.primaries == out.primaries, "primaries mismatch");
  zassert_d(in.matrix == MatrixCoefficients::RGB &&
                in.transfer == TransferCharacteristics::LINEAR,
            "must be linear RGB");
  zassert_d((out.matrix == MatrixCoefficients::REC_2020_CL ||
             out.matrix == MatrixCoefficients::CHROMATICITY_DERIVED_CL) &&
                out.transfer == TransferCharacteristics::REC_709,
            "must be 2020 CL");

  // CL is always scene-referred.
  TransferFunction func =
      select_transfer_function(out.transfer, params.peak_luminance, true);
  Matrix3x3 m = out.matrix == MatrixCoefficients::CHROMATICITY_DERIVED_CL
                    ? ncl_rgb_to_yuv_matrix_from_primaries(out.primaries)
                    : ncl_rgb_to_yuv_matrix(out.matrix);
  return std::make_unique<CLToYUVOperationC>(func.to_gamma, m[0][0], m[0][1],
                                             m[0][2], func.to_gamma_scale);
}
