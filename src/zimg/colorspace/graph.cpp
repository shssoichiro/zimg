...
template <class T> class EnumRange {
...
public:
  EnumRange(T first, T last)
      : m_first{first}, m_last{static_cast<T>(static_cast<integer_type>(last) +
                                              1)} {}
...
constexpr bool is_valid_2020cl(const ColorspaceDefinition &csp) {
  return csp.matrix == MatrixCoefficients::REC_2020_CL &&
         csp.transfer == TransferCharacteristics::REC_709;
}

constexpr bool is_valid_ictcp(const ColorspaceDefinition &csp) {
  return csp.matrix == MatrixCoefficients::REC_2100_ICTCP &&
         (csp.transfer == TransferCharacteristics::ST_2084 ||
          csp.transfer == TransferCharacteristics::ARIB_B67) &&
...
constexpr bool is_valid_csp(const ColorspaceDefinition &csp) {
  ...
  return !(csp.matrix == MatrixCoefficients::REC_2020_CL &&
           !is_valid_2020cl(csp)) &&
         !(csp.matrix == MatrixCoefficients::CHROMATICITY_DERIVED_NCL &&
           csp.primaries == ColorPrimaries::UNSPECIFIED) &&
         !(csp.matrix == MatrixCoefficients::CHROMATICITY_DERIVED_CL &&
           csp.primaries == ColorPrimaries::UNSPECIFIED) &&
         !(csp.matrix == MatrixCoefficients::REC_2100_ICTCP &&
           !is_valid_ictcp(csp)) &&
         !(csp.matrix == MatrixCoefficients::REC_2100_LMS &&
           !is_valid_lms(csp));
}

struct ColorspaceHash {
  bool operator()(const ColorspaceDefinition &csp) const {
    return std::hash<unsigned>{}((static_cast<unsigned>(csp.matrix) << 16) |
...
std::vector<ColorspaceNode>
get_neighboring_colorspaces(const ColorspaceDefinition &csp) {
  ...
  auto add_edge = [&](const ColorspaceDefinition &out_csp, auto func) {
    edges.emplace_back(out_csp,
                       std::bind(func, csp, out_csp, std::placeholders::_1,
                                 std::placeholders::_2));
  };
  ...
  if (csp.matrix == MatrixCoefficients::RGB) {
    ...
    for (auto matrix : all_matrix()) {
      if (std::find(std::begin(special_matrices), std::end(special_matrices),
                    matrix) == std::end(special_matrices))
        add_edge(csp.to(matrix), create_ncl_rgb_to_yuv_operation);
    }
    if (csp.primaries != ColorPrimaries::UNSPECIFIED)
      add_edge(csp.to(MatrixCoefficients::CHROMATICITY_DERIVED_NCL),
               create_ncl_rgb_to_yuv_operation);

    // Linear RGB can be converted to other transfer functions and primaries;
    // also to combined matrix-transfer systems.
    if (csp.transfer == TransferCharacteristics::LINEAR) {
      ...
      add_edge(csp.to(MatrixCoefficients::REC_2020_CL)
                   .to(TransferCharacteristics::REC_709),
               create_cl_rgb_to_yuv_operation);

      if (csp.primaries == ColorPrimaries::REC_2020)
        add_edge(csp.to(MatrixCoefficients::REC_2100_LMS),
                 create_ncl_rgb_to_yuv_operation);
    ...
    // LMS with ST_2084 or ARIB_B67 transfer functions can be converted to ICtCp
    // and also to linear transfer function.
    if (csp.transfer == TransferCharacteristics::ST_2084 ||
        csp.transfer == TransferCharacteristics::ARIB_B67) {
      add_edge(csp.to(MatrixCoefficients::REC_2100_ICTCP),
               create_lms_to_ictcp_operation);
      add_edge(csp.to(TransferCharacteristics::LINEAR),
               create_gamma_to_linear_operation);
    }
    // LMS with linear transfer function can be converted to RGB matrix and to
    // ARIB_B67 and ST_2084 transfer functions.
    if (csp.transfer == TransferCharacteristics::LINEAR) {
      add_edge(csp.to_rgb(), create_ncl_yuv_to_rgb_operation);
      add_edge(csp.to(TransferCharacteristics::ST_2084),
               create_linear_to_gamma_operation);
      add_edge(csp.to(TransferCharacteristics::ARIB_B67),
               create_linear_to_gamma_operation);
    }
  } else if (csp.matrix == MatrixCoefficients::REC_2100_ICTCP) {
    // ICtCp with ST_2084 or ARIB_B67 transfer functions can be converted to
    // LMS.
    if (csp.transfer == TransferCharacteristics::ST_2084 ||
        csp.transfer == TransferCharacteristics::ARIB_B67)
      add_edge(csp.to(MatrixCoefficients::REC_2100_LMS),
               create_ictcp_to_lms_operation);
...
std::vector<OperationFactory>
get_operation_path(const ColorspaceDefinition &in,
                   const ColorspaceDefinition &out) {
  if (!is_valid_csp(in) || !is_valid_csp(out))
    error::throw_<error::NoColorspaceConversion>(
        "invalid colorspace definition");
  ...
  std::unordered_map<ColorspaceDefinition, ColorspaceNode, ColorspaceHash>
      parents;

  ColorspaceDefinition vertex{};

  std::cout << "Stderr printing works\n";
